...
import heroImage from "@/assets/bio-signals-performance-hero.jpg";
...
const ProductBioSignalsPerformance = () => {
    ...
    async function fetchProduct() {
      try {
        const data = await storefrontApiRequest(PRODUCTS_QUERY, { 
          first: 10, 
          query: "title:Bio Signals Performance" 
        });
        const products = data?.data?.products?.edges || [];
        const performanceProduct = products.find((p: ShopifyProduct) => 
          p.node.title.includes("Performance")
        );
        if (performanceProduct) {
          setProduct(performanceProduct);
        }
      } catch (error) {
  ...
  const handleAddToCart = async () => {
    ...
    toast.success("Added to cart", {
      description: "Bio Signals: Performance + Recovery protocol has been added to your cart."
    });
  };
  ...
  const progressMetrics = [
    ...
    "Recovery perception and readiness",
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative w-full h-[40vh] md:h-[50vh] overflow-hidden">
        <img 
          src={heroImage} 
          alt="Bio Signals: Performance + Recovery Protocol" 
        ...
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            <div className="mb-10">
              ...
              <p className="text-sm text-muted-foreground mt-4">
                These tools are designed to support consistency and awareness.
              </p>
            </div>
            ...
            <div className="mb-10">
              ...
              <ul className="space-y-2 text-muted-foreground mb-4">
                <li className="flex items-start gap-3">
                  <span className="w-1.5 h-1.5 bg-foreground rounded-full mt-2 flex-shrink-0" />
                  Adaptation and recovery frameworks
                </li>
                <li className="flex items-start gap-3">
                  <span className="w-1.5 h-1.5 bg-foreground rounded-full mt-2 flex-shrink-0" />
                  Training stress and fatigue management
                </li>
                <li className="flex items-start gap-3">
                  <span className="w-1.5 h-1.5 bg-foreground rounded-full mt-2 flex-shrink-0" />
                  Lifestyle inputs that influence performance
                </li>
                <li className="flex items-start gap-3">
                  <span className="w-1.5 h-1.5 bg-foreground rounded-full mt-2 flex-shrink-0" />
          ...
          >
            ...
            <p className="text-muted-foreground mb-4">
              Progress is evaluated using patterns and trends, including:
            </p>
            ...
            <p className="text-sm text-muted-foreground border-l-2 border-border pl-4">
              The objective is adaptation and resilience, not forced output.
            </p>
          </motion.section>
          ...
          >
            <p className="text-muted-foreground italic">
              This protocol is designed to support performance with recovery intact — prioritizing adaptation, resilience, and sustainability over forced output.
            </p>
          </motion.section>
        </div>
      </main>
