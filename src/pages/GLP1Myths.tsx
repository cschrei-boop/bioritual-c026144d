...
import { BlogPageTemplate, AnimatedSection, BlogQuoteBlock } from "@/components/blog";
...
const GLP1Myths = () => {
  ...
  const structuredData = {
    ...
    "@graph": [
      ...
      {
        "@type": "FAQPage",
        mainEntity: myths.slice(0, 5).map((myth) => ({
          "@type": "Question",
          name: `GLP-1 Myth: ${myth.quote}`,
          acceptedAnswer: { "@type": "Answer", text: myth.explanation },
        })),
      },
    ],
  };
  ...
  return (
    <BlogPageTemplate
      ...
      hero={{
        ...
        children: (
          <Button
            variant="outline"
            size="lg"
            className="rounded-none px-8 py-6 text-sm tracking-wider border-foreground/30 hover:bg-foreground hover:text-background"
            asChild
          >
      ...
      cta={{
        ...
        title: "Signals. Not shortcuts.",
        description: "We believe in understanding biology, not fighting it. GLP-1s are one tool among many—used deliberately, not blindly.",
        buttons: [{ label: "Learn more about our approach", href: "/start-here" }],
        dark: true,
      }}
    >
      {/* Intro Section */}
