...
import heroImage from "@/assets/protocol-longevity-hero.jpg";
...
const ProductBioSignalsLongevity = () => {
    ...
    const fetchProduct = async () => {
      try {
        const data = await storefrontApiRequest(PRODUCTS_QUERY, { 
          first: 10,
          query: "title:Bio Signals Longevity"
        });
        const products = data?.data?.products?.edges || [];
        const longevityProduct = products.find((p: ShopifyProduct) => 
          p.node.title.toLowerCase().includes('longevity')
        );
        if (longevityProduct) {
          setProduct(longevityProduct);
        }
      } catch (error) {
  ...
  const educationalItems = [
    "Longevity-oriented frameworks",
    "Recovery, resilience, and adaptation concepts",
    "Lifestyle inputs that influence long-term outcomes",
    "Bio signals, including peptides, discussed in a high-level educational and informational capacity"
  ];

  const progressItems = [
    "Consistency of habits",
    "Recovery perception over time",
    "Energy and resilience patterns",
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[60vh] md:h-[70vh] overflow-hidden">
        <img 
          src={heroImage} 
          alt="Bio Signals Longevity Protocol" 
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 md:py-16">
        ...
        <motion.section className="mb-12 md:mb-16" {...fadeIn}>
          ...
          <div className="mb-10">
            ...
            <p className="text-sm text-foreground/60 italic mt-4">
              These tools are designed to support adherence and long-term continuity.
            </p>
          </div>
        ...
        <motion.section className="mb-12 md:mb-16" {...fadeIn}>
          ...
          <p className="text-foreground/80 mb-4">Progress is evaluated using long-term trends and continuity, including:</p>
          <ul className="space-y-2">
          ...
          <p className="text-sm text-foreground/60 italic mt-4">
            The objective is preservation and continuity, not short-term optimization.
          </p>
        </motion.section>
        ...
        <motion.section className="text-center py-8 border-t border-border" {...fadeIn}>
          <p className="text-foreground/80 italic max-w-2xl mx-auto">
            This protocol is designed to support long-term resilience, recovery, and functional capacity — prioritizing continuity over intensity and preservation over extremes.
          </p>
        </motion.section>
