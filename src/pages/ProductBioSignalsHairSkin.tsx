...
import heroImage from "@/assets/bio-signals-hair-skin-hero.jpg";
...
const ProductBioSignalsHairSkin = () => {
    ...
    const fetchProduct = async () => {
      try {
        const data = await storefrontApiRequest(PRODUCTS_QUERY, { 
          first: 10,
          query: "title:Bio Signals Hair"
        });
        const products = data?.data?.products?.edges || [];
        const hairSkinProduct = products.find((p: ShopifyProduct) => 
          p.node.title.toLowerCase().includes('hair') && p.node.title.toLowerCase().includes('skin')
        );
        if (hairSkinProduct) {
          setProduct(hairSkinProduct);
        }
      } catch (error) {
  ...
  const educationalItems = [
    "Skin and hair biology at a conceptual level",
    "Recovery and tissue-support frameworks",
    "Lifestyle inputs that affect tissue quality",
    "Bio signals, including peptides, discussed in a high-level educational and informational capacity"
  ];

  const progressItems = [
    "Consistency of routines",
    "Subjective changes in skin quality",
    "Observational changes over time",
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[60vh] md:h-[70vh] overflow-hidden">
        <img 
          src={heroImage} 
          alt="Bio Signals Hair + Skin Protocol" 
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 md:py-16">
        ...
        <motion.section className="mb-12 md:mb-16" {...fadeIn}>
          ...
          <div className="mb-10">
            ...
            <p className="text-sm text-foreground/60 italic mt-4">
              These tools are designed to support adherence and long-term maintenance.
            </p>
          </div>
        ...
        <motion.section className="mb-12 md:mb-16" {...fadeIn}>
          ...
          <p className="text-foreground/80 mb-4">Progress is evaluated using long-term patterns and observations, including:</p>
          <ul className="space-y-2">
          ...
          <p className="text-sm text-foreground/60 italic mt-4">
            The focus is maintenance and support, not rapid or guaranteed changes.
          </p>
        </motion.section>
        ...
        <motion.section className="text-center py-8 border-t border-border" {...fadeIn}>
          <p className="text-foreground/80 italic max-w-2xl mx-auto">
            This protocol is designed to support tissue quality, maintenance, and consistency — prioritizing long-term support over short-term fixes.
          </p>
        </motion.section>
